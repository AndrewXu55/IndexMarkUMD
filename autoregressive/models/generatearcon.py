...
def compute_green_tokens_from_context(context_tokens, vocab_size, gamma, seed):
    ...
    Args:
        context_tokens: Tensor of shape (batch_size, h) containing the previous h tokens
        vocab_size: Size of the vocabulary
        gamma: Fraction of tokens to mark as green (e.g., 0.5 for 50%)
        seed: Secret key for the hash function

    Returns:
        green_tokens: Tensor of green token indices for each batch element
    """
    if context_tokens is None or context_tokens.shape[1] == 0:
        # No context, return empty or use a default green list
        return None
    ...
    # For each batch element, hash the context to get green tokens
    green_lists = []
    for b in range(batch_size):
        # Convert context tokens to bytes for hashing
        context_str = ",".join([str(t.item()) for t in context_tokens[b]])
        hash_input = f"{seed}_{context_str}".encode('utf-8')
        hash_obj = hashlib.sha256(hash_input)

        # Use hash to seed a random generator for this context
        hash_int = int.from_bytes(hash_obj.digest()[:4], 'big')
        rng = torch.Generator(device='cpu').manual_seed(hash_int)

        # Generate green token list
        perm = torch.randperm(vocab_size, generator=rng, device='cpu')
        green_tokens_b = perm[:num_green].to(device)
        green_lists.append(green_tokens_b)
...
def sample(
    ...
    con_pairs = torch.cat(
        [token_confidence.unsqueeze(-1), paired_confidence.unsqueeze(-1)], dim=-1
    )
...
def generate(
    ...
    **sampling_kwargs
    ...
    confidences = torch.zeros(
        (max_batch_size, T_new, 2), dtype=torch.float, device=device
    )

    input_pos = torch.arange(0, T, device=device)
    next_token, first_confidence = prefill(
        model,
        cond_combined,
        input_pos,
        cfg_scale,
    ...
    generated_tokens, token_confidences = decode_n_tokens(
        model,
        next_token,
        input_pos,
        max_new_tokens - 1,
