...
def image_distortion(img_pil, seed, attack, jpeg_quality=70): 
    ...
    if attack == 'jpeg':
        ...
        crop_transform = transforms.RandomResizedCrop(img_distorted.size, scale=(0.75, 0.75), ratio=(1.0, 1.0))
        ...
        if w_erase >= w: w_erase = w
        if h_erase >= h: h_erase = h
        ...
        print(f"Warning: Unknown attack type '{attack}'. Returning original image copy.")

    return img_distorted

def load_pil_image_to_tensor(pil_image, target_device='cpu'):
    img_np_uint8 = np.array(pil_image)
    if img_np_uint8.ndim == 2: img_np_uint8 = np.stack([img_np_uint8]*3, axis=-1)
    elif img_np_uint8.shape[2] == 4: img_np_uint8 = img_np_uint8[:, :, :3]
    elif img_np_uint8.shape[2] != 3: raise ValueError(f"Unexpected number of channels: {img_np_uint8.shape[2]}")
...
def get_inner_bounding_box_indices(
    ...
    if image_size % num_grids_per_dim != 0:
        raise ValueError(f"image_size ({image_size}) must be divisible by num_grids_per_dim ({num_grids_per_dim})")
    ...
    inner_box_indices = []
    if (inner_min_grid_row <= inner_max_grid_row and
            inner_min_grid_col <= inner_max_grid_col):
...
def place_crop_and_get_indices(
    ...
    if crop_img.dim() == 4:
        if crop_img.shape[0] == 1:
            ...
            raise ValueError(f"4D crop_img batch dim must be 1, got shape {crop_img.shape}")
    elif crop_img.dim() != 3:
         raise ValueError(f"crop_img must be 3D (C, H, W) or 4D (1, C, H, W), got shape {crop_img.shape}")
    ...
    for r_offset in range(max_offset):
        for c_offset in range(max_offset):
            ...
            if h_to_place > 0 and w_to_place > 0:
                ...
                canvas[:, start_row:end_row_on_canvas, start_col:end_col_on_canvas] = crop_data_to_place
    ...
    assert len(augmented_image_list) == total_images, \
           f"Generated image count ({len(augmented_image_list)}) != expected ({total_images})"
    assert len(inner_indices_list) == total_images, \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mapping-save-path", type=str, default="path/codebook_index_mapping_knn10_mwpm.json", help="Path to save/load the index mapping JSON file")
    parser.add_argument("--pairs-save-path", type=str, default="path/codebook_pairs_knn10_mwpm.json", help="Path to save/load the codebook pair list JSON file")
    ...
    parser.add_argument("--ft-pt-path",type=str, default='path/Index_encoder.pt')
    parser.add_argument("--WATERMARK-THRESHOLD", type=float, default=0.615)
    parser.add_argument("--target-image-size", type=int, default=256)
    parser.add_argument("--chosen-attack", type=str, default='jpeg', choices=['jpeg', 'cropping', 'blurring', 'noise', 'color_jitter', 'none'])
    parser.add_argument("--jpeg-attack-quality", type=int, default=70)
    parser.add_argument("--Watermarked-dir",type=str, default='path/1000_w')
    parser.add_argument("--Not-Watermarked-dir",type=str, default='path/1000')
    ...
    if args.ft_pt_path!=None:
        finetuned_checkpoint = torch.load(args.ft_pt_path, map_location="cpu")
        ...
        print(f"Warning: Finetuned encoder checkpoint not found at {args.ft_pt_path}. Using encoder from VQ checkpoint.")
    ...
    for image_directory, true_label in directories_to_process.items():
        ...
        for idx, filename in enumerate(image_files):
            ...
            try:
                ...
                x_input = load_pil_image_to_tensor(distorted_pil_image, target_device=device)

                with torch.no_grad():
                    if args.chosen_attack == 'cropping' :
                        rate_list = []  
                        crop_size=(x_input.size(2),x_input.size(3))
                        crop_list,crop_index = place_crop_and_get_indices(args.target_image_size,x_input,device)
                    ...
                    if processed_image_count % 100 == 0:
                         print(f"  Processed {processed_image_count}/{total_images_to_process} images...")
    
    
                # Clean up memory
                if args.chosen_attack != 'cropping' :
                    del x_input, latent_orig, indices_orig, misc, original_pil_image, distorted_pil_image
                if device == 'cuda':
                    torch.cuda.empty_cache()
            except FileNotFoundError:
                print(f"Error: Image file not found at {image_path}. Skipping.")
            except Exception as e:
                print(f"Error processing image {filename} with attack '{args.chosen_attack}': {e}")
                traceback.print_exc()
                if device == 'cuda':
                     torch.cuda.empty_cache()
                gc.collect()
    ...
    print(f"\n--- Overall Results ---")
    print(f"Attack Type Applied: {args.chosen_attack}" + (f" (JPEG Quality: {args.jpeg_attack_quality})" if args.chosen_attack == 'jpeg' else ""))
    print(f"Watermark Prediction Threshold: > {args.WATERMARK_THRESHOLD}")
    print(f"Total images processed: {processed_image_count} (Expected: {total_images_to_process})")
    ...
    if processed_image_count > 0 and len(all_true_labels) == len(all_predictions):
        ...
        watermarked_indices = [i for i, label in enumerate(all_true_labels) if label == 1]
        non_watermarked_indices = [i for i, label in enumerate(all_true_labels) if label == 0]
    
        if watermarked_indices:
            watermarked_correct = sum(1 for i in watermarked_indices if all_predictions[i] == 1)
            watermarked_acc = watermarked_correct / len(watermarked_indices)
            print(f"  Accuracy on Watermarked (Label 1): {watermarked_acc:.4f} ({watermarked_correct}/{len(watermarked_indices)})")
    
        if non_watermarked_indices:
            non_watermarked_correct = sum(1 for i in non_watermarked_indices if all_predictions[i] == 0)
            non_watermarked_acc = non_watermarked_correct / len(non_watermarked_indices)
            print(f"  Accuracy on Non-Watermarked (Label 0): {non_watermarked_acc:.4f} ({non_watermarked_correct}/{len(non_watermarked_indices)})")
